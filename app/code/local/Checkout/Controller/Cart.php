...
class Checkout_Controller_Cart extends Core_Controller_Front_Action{
    ...
    public function addAction(){
        ...
        Mage::getSingleton('checkout/session')
            ...
            ->save();
        // echo '<pre>';
        // print_r($this->getRequest()->getMessageModel());
        // echo '</pre>';die;
        $this->getRequest()->getMessageModel()->addSuccess("Product added to cart successfully!🛒");
        // $this->redirect('checkout/cart/index');
        $this->redirect('catalog/product/view/?id='.$productId);


    }

    public function dummyAction(){
        $this->getRequest()->getMessageModel()->addSuccess("hello babe");
        print_r ($this->getRequest()->getMessageModel()->getSuccessMessage());
    ...
    public function placeOrderAction(){
        ...
        if($cart->getPaymentMethod() == 'paypal'){
            $this->redirect('payment/paypal/start');
        } else {
            $this->redirect('checkout/cart/convertOrder');
        }
    ...
    public function convertOrderAction(){
        ...
        $transaction_id = $this->getRequest()->getQuery('transaction_id');
        $cart = Mage::getSingleton('checkout/session')
            ->getCart();
